...
export default function Competencias() {
  ...
  if (isLoading)
    return (
      <div className="loading-data-screen">
        <span>Loading...</span>
      </div>
    );
  ...
  const listaDesarrolloWeb = competenciasData.web.map((tecnologia) => {
    return (
      <div key={tecnologia.title} className={style.materia}>
        ...
        <ul>
          {tecnologia.temas.map((tema, i) => (
            <li key={i}>{he.decode(tema)}</li>
          ))}
        </ul>
      </div>
    );
    ...
    <main className="page_container">
      ...
      <main className={style.contenedor_listas}>
        <section className={style.contenedor_lista}>
          <h1>WEB</h1>
          <div className={style.materias}>{listaDesarrolloWeb}</div>
        </section>

        <section className={style.contenedor_lista}>
          <h1>FACULTAD</h1>
          <div className={style.materias}>{listaFacultad}</div>
        </section>
      </main>
    </main>
  );
