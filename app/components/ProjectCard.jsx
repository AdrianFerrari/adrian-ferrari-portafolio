...
export default function ProjectCard({ title, id, description, tags, screenshots, links }) {
  return (
    <div className={style.project_card + " " + style[id]}>
      ...
      <div className={style.info}>
        ...
        <div className={style.info_tags}>
          {tags.map((tag) => {
            return (
              <div className={style.info_tag} key={tag}>
                {tag}
              </div>
            );
          })}
      ...
      <div className={style.buttons}>
        {links.map((item) => {
          return (
            <Link target="_blank" href={item.link} key={item.name} className={style[item.name.toLowerCase()]}>
